...
def inference(cfg):
    ...
    print(f"Input video path = {cfg.video_path}")
    ...
    images, _, _ = read_video(cfg.video_path, output_format='TCHW')
    images = images.float()
    ...
    flow = torch.stack(results)
    torch.save(flow, cfg.save_path)
    # N = len(results)
    # vis_flow = []
    # for idx in range(N):
    #     flow_img = flow_viz.flow_to_image(results[idx].permute(1, 2, 0).numpy())
...
if __name__ == '__main__':
    ...
    parser.add_argument('--video_path', default='default')
