...
def register_extended_stem_attention_pnp(model, injection_schedule):
    def sa_forward(self):
        ...
        def forward(x, encoder_hidden_states=None, attention_mask=None):
            ...
            k_cond, v_cond = em(k_cond, v_cond, num_bases=64)
...
# ===================================== STEM ===========================================
def register_extended_stem_attention(model, opt=None):
    def sa_forward(self):
        to_out = self.to_out
        if type(to_out) is torch.nn.modules.container.ModuleList:
            to_out = self.to_out[0]
        ...
        def forward(x, encoder_hidden_states=None, attention_mask=None):
            ...
            # k = self.head_to_batch_dim(k).view(n_frames, h, sequence_length, dim // h)   # h, n, c//h
            # v = self.head_to_batch_dim(v).view(n_frames, h, sequence_length, dim // h)
            ...
            k, v = em(k, v, num_bases=64)  # h, m, c//h
...
# ===================================== GLFM ===========================================
def register_extended_glfm_attention(model, opt):
    def sa_forward(self):
        to_out = self.to_out
        if type(to_out) is torch.nn.modules.container.ModuleList:
            to_out = self.to_out[0]
        ...
        def forward(x, encoder_hidden_states=None, attention_mask=None):
            ...
            # anchor
            # ++++++++++++++++++++++++++++++++++++++++ glfm +++++++++++++++++++++++++++++++++++++++++
            clip_n_frms = opt.n_clip_frames
            q = self.to_q(x)
            k = self.to_k(encoder_hidden_states)
            v = self.to_v(encoder_hidden_states)
            v_cp = v.clone() # (n_frms, seq_len, c)
            k_cp = k.clone()

            # ------------------------------ global frames modeling -----------------------------------
            k = k.reshape(1, n_frames, sequence_length, -1).flatten(start_dim=1, end_dim=2)  # 1, n, c
            v = v.reshape(1, n_frames, sequence_length, -1).flatten(start_dim=1, end_dim=2)  # 1, n, c'
            ...
            k, v = em(k, v, num_bases=opt.gfm_k1)  # h, m, c//h

            k = k.view(1, h, -1, dim // h).repeat(n_frames, 1, 1, 1)
            v = v.view(1, h, -1, dim // h).repeat(n_frames, 1, 1, 1)
            q = self.head_to_batch_dim(q).view(n_frames, h, sequence_length, dim // h)
        
            # ------------------------------ local frames modeling --------------------------------
            k_clips, v_clips = [], []
            for i in range(0, len(x), clip_n_frms):
                v_clip = v_cp[i:i+clip_n_frms]
                k_clip = k_cp[i:i+clip_n_frms]
                nfs, _, _ = v_clip.shape
                v_clip = v_clip.reshape(1, nfs, sequence_length, -1).flatten(start_dim=1, end_dim=2)
                k_clip = k_clip.reshape(1, nfs, sequence_length, -1).flatten(start_dim=1, end_dim=2)
                v_clip = self.head_to_batch_dim(v_clip)
                k_clip = self.head_to_batch_dim(k_clip)
                k_clip, v_clip = em(k_clip, v_clip, num_bases=opt.lfm_k2)
                v_clip = v_clip.view(1, h, -1, dim // h).repeat(nfs, 1, 1, 1)
                k_clip = k_clip.view(1, h, -1, dim // h).repeat(nfs, 1, 1, 1)#.unsqueeze(1)
            
                k_clips.append(k_clip)
                v_clips.append(v_clip)
            # ------------------------------ spatial-temporal fusion --------------------------------
            k_clips = torch.cat(k_clips, dim=0)
            v_clips = torch.cat(v_clips, dim=0)
            all_scale = opt.gfm_alpha
            clip_scale = opt.lfm_beta
            k = torch.cat([k*all_scale, k_clips*clip_scale], dim=-2) # (n_frames, h, k1+k2, c//h)
            v = torch.cat([v*all_scale, v_clips*clip_scale], dim=-2) # (n_frames, h, k1+k2, c//h)
