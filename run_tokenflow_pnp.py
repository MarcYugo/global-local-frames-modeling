...
class TokenFlow(nn.Module):
    def __init__(self, config, opt):
        ...
        if sd_version == '2.1':
            model_key = f"{opt.models_dir}/stabilityai/stable-diffusion-2-1-base"
        elif sd_version == '2.0':
            model_key = f"{opt.models_dir}/stabilityai/stable-diffusion-2-base"
        elif sd_version == '1.5':
            model_key = f"{opt.models_dir}/runwayml/stable-diffusion-v1-5"
        elif sd_version == 'depth':
            model_key = f"{opt.models_dir}/stabilityai/stable-diffusion-2-depth"
        elif sd_version == 'xl':
            model_key = f"{opt.models_dir}/stabilityai/stable-diffusion-xl-base-0.9"
        else:
    ...
    def batched_denoise_step(self, x, t, indices):
        batch_size = self.config["flow_batch_size"] #20
        # key_frm_interval = self.config["key_frame_interval"]
        denoised_latents = []
        # pivotal_idx = torch.randint(key_frm_interval, (len(x)//key_frm_interval,)) + torch.arange(0,len(x),key_frm_interval) 
        ...
        # for i in range(len(pivotal_idx)):
        #     self.denoise_step(x[pivotal_idx[i:i+1]], t, indices[pivotal_idx[i:i+1]])
    ...
    def sample_loop(self, x, indices):
        ...
        # x_slices = []
        decode_lates = []
        # for i in range(0,len(x), DENOISE_BATCH):
        #     # print(f'Denoise batch: {i//DENOISE_BATCH} of {len(x)//DENOISE_BATCH}')
        #     x_batch = x[i:i+DENOISE_BATCH]
        #     indices_batch = indices[i:i+DENOISE_BATCH]
        #     for i, t in enumerate(tqdm(self.scheduler.timesteps, desc=f"Sampling batch: {i//DENOISE_BATCH} of {len(x)//DENOISE_BATCH}")):
        #         x_batch = self.batched_denoise_step(x_batch, t, indices_batch)
        #     x_slices.append(x_batch.detach().cpu())
        for i, t in enumerate(tqdm(self.scheduler.timesteps, desc="Sampling")):
            x = self.batched_denoise_step(x, t, indices)
        if len(x) > DENOISE_BATCH:
            x_slices = x.chunk(len(x)//DENOISE_BATCH)
        else:
            x_slices = [x]
        for i,slice in enumerate(x_slices):
            print(f'Decode batch: {i+1} of {len(x_slices)}')
            tmp_decode_latent = self.decode_latents(slice.to(device))
            decode_lates.append(tmp_decode_latent.detach().cpu())
        # decoded_latents = self.decode_latents(x)
        decoded_latents = torch.cat(decode_lates, dim=0)
