...
class Preprocess(nn.Module):
    def __init__(self, device, opt, hf_key=None):
        ...
        if self.sd_version == 'ControlNet':
            ...
            controlnet = ControlNetModel.from_pretrained(f"{opt.models_dir}lllyasviel/sd-controlnet-canny", torch_dtype=torch.float16).to(self.device)
            control_pipe = StableDiffusionControlNetPipeline.from_pretrained(
                f"{opt.models_dir}runwayml/stable-diffusion-v1-5", controlnet=controlnet, torch_dtype=torch.float16
            ).to(self.device)
    ...
    def init_method(self):
        register_extended_glfm_attention(self, self.opt)
...
if __name__ == "__main__":
    ...
    opt = edict(config)
    opt.models_dir = args.models_dir
